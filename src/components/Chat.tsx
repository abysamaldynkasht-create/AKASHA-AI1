...
import { getAIResponseStream } from '../lib/gemini';
import { getUserMemory, updateLongTermMemory } from '../lib/memoryService';
import { Send, Bot, User as UserIcon, Loader2, Brain, Sparkles } from 'lucide-react';
...
export const Chat: React.FC<ChatProps> = ({ sessionId, onSessionCreated }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || "حدث خطأ غير متوقع أثناء الاتصال بالنموذج.");
    } finally {
      setIsLoading(false);
      setStreamingText(null);
    }
    ...
    <div className="flex flex-col h-full bg-bg-dark text-[#F5F5DC] relative" dir="rtl">
      <div className="flex-1 overflow-y-auto p-4 md:p-8 lg:p-12 space-y-6 md:space-y-8 custom-scrollbar">
        {messages.length === 0 && !isLoading && !streamingText && (
          <div className="h-full flex flex-col items-center justify-center text-center space-y-4 md:space-y-6 opacity-80 px-4">
            ...
            <div className="space-y-2">
              <h2 className="text-2xl md:text-3xl font-black tracking-tighter">أهلاً بك في Akasha AI</h2>
              <p className="max-w-md text-[#A0A0A0] font-medium text-sm md:text-base">
                متصل الآن بنموذج KAI-1 فائق الذكاء مع الرد المتدفق الفوري. كيف يمكنني مساعدتك اليوم؟
              </p>
            </div>
        ...
        {isLoading && (
          ...
          >
            <div className="w-10 h-10 rounded-xl bg-primary flex items-center justify-center flex-shrink-0 mt-1 shadow-lg overflow-hidden text-white">
              {logoUrl && !logoError ? (
                <img 
                  src={logoUrl} 
                  alt="AI" 
            ...
            <div className="max-w-[90%] md:max-w-[85%] lg:max-w-[75%] p-4 md:p-5 rounded-2xl md:rounded-3xl shadow-xl bg-white/5 text-[#F5F5DC] rounded-tr-none border border-white/5 backdrop-blur-sm">
              {streamingText ? (
                <div className="prose prose-invert text-sm md:text-base">
      ...
      <div className="p-4 md:p-8 lg:p-12 bg-gradient-to-t from-bg-dark via-bg-dark to-transparent">
        ...
        <div className="text-center mt-6 space-y-2">
          <p className="text-[8px] md:text-[10px] text-[#404040] uppercase tracking-[0.2em] md:tracking-[0.4em] font-black flex items-center justify-center gap-1.5">
            <Sparkles size={10} className="text-primary" />
            Akasha AI - KAI-1 Streaming Model
          </p>
